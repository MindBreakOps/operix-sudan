...
export default function TechMarquee() {
  ...
  return (
	...
	<section className="relative overflow-hidden border-y border-vercel-border bg-vercel-surface py-5 flex items-center" dir="ltr">
	  ...
	  <div className="flex w-max items-center animate-marquee hover:[animation-play-state:paused]">
		{[...products, ...products, ...products, ...products].map((p, i) => (
		  <a 
			...
			target="_blank" 
			rel="noreferrer"
			dir={isAr ? "rtl" : "ltr"} // Keeps the icon and text order correct inside the button
		  ...
		  >
			...
			<ExternalLink size={14} className="text-white/80 group-hover:text-white group-hover:-translate-x-1 transition-all rtl:group-hover:-translate-x-1 ltr:group-hover:translate-x-1" />
		  </a>
		))}
	  </div>
	</section>
