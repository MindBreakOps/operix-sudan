...
export default function TechMarquee() {
  return (
	<section className="relative overflow-hidden border-y border-vercel-border bg-vercel-surface py-5 flex items-center">
	  ...
	  {/* Using the native CSS class for guaranteed scrolling */}
	  <div className="native-marquee items-center">
		{[...products, ...products, ...products, ...products].map((p, i) => (
		  <a 
			...
			target="_blank" 
			rel="noreferrer"
			className="mx-6 flex items-center gap-4 bg-vercel-bg border border-vercel-border px-5 py-2.5 rounded-full hover:border-brand-gold hover:bg-vercel-hover transition-all duration-300 cursor-pointer shadow-sm group"
		  >
			<div className="bg-vercel-surface p-1.5 rounded-full border border-vercel-border group-hover:border-brand-gold transition-colors">
			  <p.icon size={14} className="text-vercel-muted group-hover:text-brand-gold transition-colors" />
			</div>
			<span className="text-sm font-bold text-vercel-surface-text tracking-wide">{p.name}</span>
			<span className="text-xs font-mono text-vercel-muted flex items-center gap-1.5">
			  www.{p.url}
